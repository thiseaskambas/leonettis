...
| POST   | `/api/admin/listings`      | Create. Required: `title.en`, `listingType`, `propertyTypes`. Also stores legacy primary `propertyType` from the first selected type. Generates `id` (UUID), `publishedAt`, `updatedAt`. Uses request `slug` when provided (slugified); otherwise derives slug from `title.en`. Defaults: `status: active`, `isFeatured/favorite/urgent: false`, arrays default to `[]`. Returns `201 { listing }`.      |
