...
| Variable               | Required | Description                                                 |
| ---------------------- | -------- | ----------------------------------------------------------- |
| `NEXT_PUBLIC_SITE_URL` | Yes      | Canonical public site origin used for metadata, sitemap, and robots URLs |
...
| `propertyType` | `PropertyType`           | Legacy primary value, set from the first `propertyTypes` entry     |
| `propertyTypes`| `PropertyType[]`         | Array — a listing can span multiple property types                 |
...
| POST   | `/api/admin/listings`      | Create. Required: `title.en`, `listingType`, `propertyTypes`. Also stores legacy primary `propertyType` from the first selected type. Generates `id` (UUID), `publishedAt`, `updatedAt`. Uses a non-empty request `slug` when provided (slugified and validated); otherwise derives slug from `title.en`. Duplicate or invalid slugs return `400`. Defaults: `status: active`, `isFeatured/favorite/urgent: false`, arrays default to `[]`. Returns `201 { listing }`.      |
...
POST /api/admin/listings/[id]/media/presign
  ...
  create signed PutObject URL and deterministic media key
              │
              ▼
Browser PUT signedUploadUrl (file bytes sent directly to Sevalla; admin UI uses XMLHttpRequest for videos so upload.onprogress can drive a real progress bar; images keep fetch where applicable)
              │
              ▼
